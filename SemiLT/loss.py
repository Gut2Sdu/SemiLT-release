...
def rna_reduction(embedding, rna_label, identity_matrix, rare):
    ...
    rna_embedding_loss =  - w * celltype_dist_mean.mean() +  ( 1 - w ) / embedding.std(dim=0).mean() + rna_embedding(embedding,rna_label) #+ 1 / rna_mean.std(dim=0).mean()
    loss = rna_embedding_loss + rna_other_loss #rna_embedding_loss +  #+ rna_cluster_loss +  rna_common_loss
    
    return loss

def Compute_KL(atac_embedding_cat,rna_embedding_cat):
    kl_loss = nn.KLDivLoss(reduction = 'batchmean')
    mu1 = torch.softmax(rna_embedding_cat.mean(dim = 0),dim = 0)
    mu2 = torch.softmax(atac_embedding_cat.mean(dim = 0),dim = 0)
    kl = kl_loss(mu1.log(),mu2)
    seigema1 = torch.softmax(rna_embedding_cat.std(dim = 0),dim = 0)
    seigema2 = torch.softmax(atac_embedding_cat.std(dim = 0),dim = 0)
    kl = (kl + kl_loss(seigema1.log(),seigema2))/2
    return kl
...
def remove_batch(expression_batch1, expression_batch2):
    ...
    mean_diff = (mean_batch1 - mean_batch2).abs().mean() #mean_batch1.abs().mean() + mean_batch2.abs().mean() #
    var_diff = (var_batch1 - var_batch2).abs().mean() #(1-var_batch1.abs().mean()).abs() + (1 - var_batch2.abs().mean()).abs() #
...
class EncodingLoss(nn.Module):
    ...
    def forward(self, atac_embeddings, rna_embeddings,rna_labels,peak_data):
        ...
        # rna_atac_loss = embedding_loss(atac_embedding_cat,rna_embedding_cat,rna_label)
        ...
        # rna_mean_r = rna_mean_sim[~torch.isclose(rna_mean_sim, torch.tensor([1.]), rtol=1e-05, atol=1e-08)]
        # L1_loss = 0
        # # count = 1
        # for i in range(len(total_k_sim[0])):
        #     if total_k_sim[0][i] > rna_mean_sim.mean():  
        #         L1_loss += (atac_embedding_cat[total_k_sim[1][i],:] - rna_mean[int(cos_k_maxid[total_k_sim[1][i]]),:]).abs().mean() #total_k_sim[0][i].detach() * 
        ...
        # peak_knn_graph[peak_knn_graph < torch.quantile(peak_knn_graph[peak_knn_graph > 0], 0.03)] = 0 #
        ...
        if len(set(atac_pre)) > 1:
            ...
            _,atac_pre_dist = cell_cluster_dist(atac_embedding_pre,torch.Tensor(atac_pre).int())
        ...
        batch_loss = remove_batch(atac_embedding_cat, rna_embedding_cat) #+ Compute_KL(atac_embedding_cat,rna_embedding_cat) # -cosine_sim(atac_embedding_cat, rna_embedding_cat).mean() + 
    
        
        loss =  rna_reduction_loss + atac_reduction_loss + atac_dist_loss + near_loss + batch_loss + mnn_loss + 1.5 * L_mnn
        # loss =  0.3 * (atac_reduction_loss + rna_reduction_loss + near_loss) + mnn_loss #+ 0.25 * near_loss + L1_weight * L1_loss #+ 0.25 * near_loss  + L1_weight*L1_loss  #rna_atac_loss
        # loss =  0.3 * (atac_reduction_loss + rna_reduction_loss + near_loss ) + mnn_loss #+ L1_weight * L1_loss #
        return loss
