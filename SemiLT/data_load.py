...
class PrepareDataloader():
    def __init__(self, setting):
        ...
        # load data without peak
            ...
            self.num_of_atac = 0
            if len(setting.atac_paths) == len(setting.atac_protein_paths):
                for atac_path, atac_protein_path in zip(setting.atac_paths, setting.atac_protein_paths):  
                    data_reader, _,_, protein_reader, _ = read_from_file(atac_path, atac_protein_path)
                    # train loader
                    trainset = DataloaderWithoutLabel(True, data_reader, None, protein_reader,peak_reader)
                    self.num_of_atac += len(trainset)
                    trainloader = torch.utils.data.DataLoader(trainset, batch_size=
                ...
                for atac_path  in zip(setting.atac_paths):   
                    data_reader, _, _, _, _ = read_from_file(atac_path[0],None,None)
                    # train loader
                    trainset = DataloaderWithoutLabel(True, data_reader,None,None,peak_reader)
                    self.num_of_atac += len(trainset)
